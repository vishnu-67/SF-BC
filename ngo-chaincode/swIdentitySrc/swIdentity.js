 ...
 */
async function queryByStringSWIdentity(stub, queryString) {
    console.log('============= START : queryByStringSWIdentity ===========');
    console.log("##### queryByStringSWIdentity queryString: " + queryString);
    ...
    if (jsonQueryString['selector'] && jsonQueryString['selector']['docType']) {
        ...
        throw new Error('##### queryByStringSWIdentity - Cannot call queryByString without a docType element: ' + queryString);
    }

    let iterator = await stub.getHistoryForKey(docType);
    ...
    while (true) {
        ...
        if (res.value && res.value.value.toString()) {
            let jsonRes = {};
            console.log('##### queryByStringSWIdentity iterator: ' + res.value.value.toString('utf8'));
            ...
            console.log('##### queryByStringSWIdentity jsonRecord - number of JSON keys: ' + Object.keys(jsonRecord).length);
            ...
            for (var key in jsonRecord) {
                if (jsonRecord.hasOwnProperty(key)) {
                    console.log('##### queryByStringSWIdentity jsonRecord key: ' + key + " value: " + jsonRecord[key]);
                    if (key == "docType") {
                        continue;
                    }
                    console.log('##### queryByStringSWIdentity json iterator has key: ' + jsonRes.Record[key]);
        ...
        if (res.done) {
            ...
            console.log('##### queryByStringSWIdentity all results: ' + JSON.stringify(allResults));
            console.log('============= END : queryByStringSWIdentity ===========');
...
let Chaincode = class {
    ...
    async initLedger(stub, args) {
        ...
        /************************************************************************************************
         * 
         * Self Profile Identity functions 
         * 
         ************************************************************************************************/

    /**
     * Creates a new Self Profile Identity
     * 
     * @param {*} stub 
     ...
     *   "state": "Wisconsin",
     *   "County": "Kanosha",
     *   "authToken": "token73938883",
     *   "FCMToken": "7839FF0033",
     *   "phoneNo": "9999999999",
     *   "timestamp": "1611088000000"
     * }
     */
    async createSWIdentity(stub, args) {
        console.log('============= START : createSWIdentity ===========');
        console.log('##### createSWIdentity arguments: ' + JSON.stringify(args));
    ...
    /**
     * Retrieves a SWecfic Self Profile Identity
     * 
     * @param {*} stub 
     * @param {*} args 
     */
    async querySWIdentity(stub, args) {
        console.log('============= START : querySWIdentity ===========');
        console.log('##### querySWIdentity arguments: ' + JSON.stringify(args));
    ...
    /**
     * Retrieves all Self Profile Identity
     * 
     * @param {*} stub 
     * @param {*} args 
     */
    async queryAllSWIdentityHist(stub, args) {
        console.log('============= START : queryAllSWIdentity ===========');
